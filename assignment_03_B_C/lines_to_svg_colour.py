...
purpose
	parse line
	if legal
		return [x0, y0, x1, y1, colour] as ints, str
	else
		...
		if m:
			...
			if offset == len(line): # no more tokens remaining
				if len(L) < 4: # error: too few x/y values
					return offset
				if colour != None and colour != '' and colour != 'None': # if colour exists
					if colour in accepted_colors:
						L.append(colour)
						return L
				else: # otherwise use default colour
					L.append('Black')
					return L
			else:
				if len(L) == 4:
...
global accepted_colors
accepted_colors = []
# create list of acceptable colours for SVG
try:
	colour_file = open('css_colours.txt')
	for line in colour_file:
		if line[-2:] == '\r\n': # Windows
			accepted_colors.append(line[:-2]) # strip carriage return and newline
		elif line[-1] == '\n': # Linux
			accepted_colors.append(line[:-1]) # strip newline
except IOError:
